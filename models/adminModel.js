...
class adminModel {
    ...
    search(obj) {
        return new Promise((resolve, reject) => {
            let { username } = obj;
            db.query("select * from admin_table where username like ?", ["%" + username + "%"], (err, results) => {
                // 执行数据库语句错误
                console.log(results);
                if (err) {
    ...
    delete(obj) {
        ...
        new Promise((resolve,reject)=>{
            db.query(`delete from admin_table where id =?`,[id], (err, results) => {
                if (err) {
                    reject({ status: 1, message: err.message, time: 3 })
                    return;
                }
                if (results.affectedRows !== 1) {
                    resolve({ status: 1, message: '删除管理员失败！', time: 3 })
                    return;
                }
                resolve({ status: 0, message: '删除管理员成功！', time: 3 })
            })
        })
    }
    delete_more(obj) {
        ...
        // return ;
        return new Promise((resolve,reject)=>{
            db.query(`delete from admin_table where id in ${ids}`, (err, results) => {
                if (err) {
                    console.log(1,err);
                    reject({ status: 1, message: err.message, time: 3 })
                    return;
